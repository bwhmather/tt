...
tt_behaviour_selector_do_call(
    ...
    while (state->active_child < behaviour->num_children) {
        ...
        if (result == TT_BEHAVIOUR_SUCCEEDED) {
            return result;
        }
...
static TTBehaviourResult
tt_behaviour_selector_do_resume(
    TTBehaviour *base, TTEntityId entity_id, void *fp
) {
    TTBehaviourLoop *behaviour = (TTBehaviourLoop *)base;
    TTBehaviourLoopState *state = (TTBehaviourLoopState *)fp;
...
static void
tt_behaviour_selector_do_interrupt(
    TTBehaviour *base, TTEntityId entity_id, void *fp
) {
    TTBehaviourLoop *behaviour = (TTBehaviourLoop *)base;
    TTBehaviourLoopState *state = (TTBehaviourLoopState *)fp;
...
static size_t
tt_behaviour_selector_max_stack_size(TTBehaviour *base) {
    TTBehaviourLoop *behaviour = (TTBehaviourLoop *)base;

    size_t max_child_stack_size = 0;
    for (size_t child = 0; child < behaviour->num_children; child++) {
        size_t child_stack_size =
            tt_behaviour_max_stack_size(behaviour->children[child]);
...
static void
tt_behaviour_selector_free(TTBehaviour *base) {
    TTBehaviourLoop *behaviour = (TTBehaviourLoop *)base;
...
TTBehaviour *
tt_behaviour_selector_from_array(TTBehaviour *children[]) {
    size_t num_children;
    for (num_children = 0; children[num_children] != NULL; num_children++) {
    }

    TTBehaviourLoop *behaviour = (TTBehaviourLoop *)malloc(
        sizeof(TTBehaviourLoop) + sizeof(TTBehaviour *) * (num_children + 1)
    );
    tt_assert(behaviour != NULL);

    behaviour->behaviour = (TTBehaviour
    ){.do_call = tt_behaviour_selector_do_call,
      .do_resume = tt_behaviour_selector_do_resume,
      .do_interrupt = tt_behaviour_selector_do_interrupt,

      .frame_size = sizeof(TTBehaviourLoopState),
      .compute_max_stack_size = tt_behaviour_selector_max_stack_size,

      .free = tt_behaviour_selector_free};

    behaviour->num_children = num_children;
    memcpy(
        &behaviour->children, children, num_children * sizeof(TTBehaviour *)
    );

...
void
tt_bitset_init(TTBitset *bitset) {
...
void
tt_bitset_destroy(TTBitset *bitset) {
    tt_assert(bitset != NULL);
...
void
tt_bitset_clear(TTBitset *bitset) {
    tt_assert(bitset != NULL);

    if (bitset->storage == NULL)
        return;
...
void
tt_bitset_copy(TTBitset *src, TTBitset *tgt) {
    tt_assert(src != NULL);
    tt_assert(tgt != NULL);
    ...
    if (tgt->capacity != src->capacity) {
        tgt->storage =
            (uint64_t *)realloc(tgt->storage, sizeof(uint64_t) * block_count);
...
void
tt_bitset_set(TTBitset *bitset, TTEntityId entity_id) {
    tt_assert(bitset != NULL);
    ...
    if (entity_id >= bitset->capacity) {
        ...
        size_t old_block_count =
            bitset->capacity ? ((bitset->capacity - 1) / 64) + 1 : 0;
        ...
        memset(
            &bitset->storage[old_block_count], 0,
            sizeof(uint64_t) * (new_block_count - old_block_count)
        );
...
void
tt_bitset_unset(TTBitset *bitset, TTEntityId entity_id) {
    tt_assert(bitset != NULL);
...
bool
tt_bitset_get(TTBitset *bitset, TTEntityId entity_id) {
    tt_assert(bitset != NULL);
    ...
    if (!(bitset->storage[entity_id / 64] & (((uint64_t)1) << (entity_id % 64))
        )) {
        return false;
    }

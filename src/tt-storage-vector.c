...
static void
on_delete_callback(TTEntityId entity_id, void *data) {
...
tt_storage_vector_new(size_t component_size) {
    TTStorageVector *storage =
        (TTStorageVector *)malloc(sizeof(TTStorageVector));
    ...
    storage->mask =
        (uint64_t *)calloc(sizeof(uint64_t), ((storage->count - 1) / 64) + 1);
    tt_assert(storage->mask != NULL);

    storage->buffer = (unsigned char *)calloc(component_size, storage->count);
    tt_assert(storage->buffer != NULL);

    storage->delete_callback_handle =
        tt_entities_bind_on_delete_callback(&on_delete_callback, storage);
...
tt_storage_vector_has(TTStorageVector *storage, TTEntityId entity_id) {
    ...
    if (!(storage->mask[entity_id / 64] & (((uint64_t)1) << (entity_id % 64))
        )) {
        return false;
    }
...
void *
tt_storage_vector_add(TTStorageVector *storage, TTEntityId entity_id) {
    tt_assert(storage != NULL);
    ...
    if (entity_id >= storage->count) {
        ...
        memset(
            &storage->mask[old_mask_count], 0,
            sizeof(uint64_t) * (new_mask_count - old_mask_count)
        );
        ...
        memset(
            &storage->buffer[storage->size * storage->count], 0,
            storage->size * (new_count - storage->count)
        );
...
void *
tt_storage_vector_get(TTStorageVector *storage, TTEntityId entity_id) {
    tt_assert(storage != NULL);
...
void
tt_storage_vector_remove(TTStorageVector *storage, TTEntityId entity_id) {
    tt_assert(storage != NULL);
